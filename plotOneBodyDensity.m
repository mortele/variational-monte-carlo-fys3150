...
% positions = load('../VMC-Release/oneBody.dat');
% nParticles  = positions(1,1);
% nDimensions = positions(2,1);
% positions = positions(3:end,:);
% N = size(positions, 1);

x = zeros(N*nParticles, 1);
y = zeros(N*nParticles, 1);

for i=1:1
    x((i-1)*N+1:i*N) = positions(:,1+(i-1)*nDimensions);
    y((i-1)*N+1:i*N) = positions(:,2+(i-1)*nDimensions);
end
    
colormap('gray');
[z, N] = hist3([x y], [50 50]);

xb = linspace(min(x),max(x),size(z,1)+1);
yb = linspace(min(y),max(y),size(z,1)+1);

imagesc(xb, yb, max(max(sqrt(z)))-sqrt(z));
